...
class BM25Retriever:
    ...
    def build_index(self, corpus_texts: List[str]):
        ...
        # Tokenize each document in corpus_texts
        ...
        # Create BM25Okapi index using tokenized_corpus
    ...
    def retrieve(self, query_texts: List[str], k: int = 20) -> Dict[int, List[int]]:
        ...
        for q_idx, query in enumerate(tqdm(query_texts, desc="Retrieving")):
            # Tokenize the query (same as corpus tokenization)
            tokenized_query = query.lower().split()
            
            # Get BM25 scores for all documents using tokenized_query
            doc_scores = self.bm25.get_scores(tokenized_query)
            
            # Find top-k document indices with highest scores
